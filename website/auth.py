...
def sign_up():
    if request.method == 'POST':
        ...
        if not image_file:
            ...
            blob_client = blob_service_client.get_blob_client(container="imgs", blob = request.form.get('username') + '-face.jpeg')
        ...
        if user:
            ...
            new_user = User(image_file=secure_filename(image_file.filename), image_data=image_file.read(), image_mimetype=image_mimetype, email=email, username=username, password=generate_password_hash(pass1, method='sha256'))
